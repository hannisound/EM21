<section id="ftipps">
	<div class="container clearfix">
    	<div class="row clearfix">
        	<div class="col-lg-10 mx-auto clearfix">
            <?php
				if (isset($_GET['grp'])) {
					if ($_GET['grp'] == "Achtelfinale" || $_GET['grp'] == "Viertelfinale" || $_GET['grp'] == "Halbfinale" || $_GET['grp'] == "Spiel um Platz 3" || $_GET['grp'] == "Finale") {
					}
					else {
				...
				//Prüfen wenn ErgebnisSend abgesetzt wurden
					if (isset($_POST['ErgebnisSend'])) {
						...
						foreach ($ergebnis1 as $key) {
							// Tipps einragen
							saveErgebnis($key[0], $key[1], $key[2], $mysqli, $key[3]);
						}
						// Last Place speichern
							saveLastPlace($mysqli);
						// User Punkte Vorrunde Eintragen
							userPointsGames($mysqli);

						echo '<br><div class="alert alert-success text-center"><strong>Ihre Tipps wurden Erfolgreich übernommen</strong></div>';
					}
			...
			<ul class="nav nav-pills nav-fill">
				<li class="nav-item">
					<a class="nav-link <?php echo $active; ?>" href="index.php?site=ftipps">Alle</a>
			</li>

			<?php
				$active1 = "";
				$sql = "SELECT grpID, name FROM ".prefix."_grp WHERE grpID >= '9'";
				...
				while ($grp = $result->fetch_array()) {
					...
					echo "<li class='nav-item'>
					  		<a class='nav-link $active1' href='index.php?site=ftipps&grp=".$grp['name']."'>".$grp['name']."</a>
						</li>";
				}
							?>
			...
			<?php echo "<div class='col justify-content-end text-right' aria-label='Tipps speichern'>
			...
			// Gefixt: Gruppen anpassen an EM21 (Kein Spiel um Platz 3)
			if (isset($_GET['grp']) && $_GET['site'] == 'ftipps') {
				$gruppen = $_GET['grp'];
				$gruppe = array("9" => "Achtelfinale",
								"10" => "Viertelfinale",
								"11" => "Halbfinale",
								"13" => "Finale");
			...
			while ($game = $result->fetch_array()) {
				...
				if ($_SESSION['status'] == 3) {
					...
					echo "<div class='col-md-auto col-sm text-center'>". $game['ergebnis_1']. " : ". $game['ergebnis_2']. " <br>(". $game['h_ergebnis_1']. " : ". $game['h_ergebnis_2']. ")</div>";
					$saveErgebnis = "";
				}
